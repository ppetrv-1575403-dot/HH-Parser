...
function onCityInput(value) {
     clearTimeout(cityTimeout);
     console.info("onCityInput " + value)
     cityTimeout = setTimeout(() => loadCities(value), 300);
}

async function loadCities(query) {
    if (query.length < 2) return;
    try {
        console.info("start loadCities")
...
async function startParsing(formData) {
    console.info('startParsing:', formData);
    try {
...
function initSearchTextInput(input) {
